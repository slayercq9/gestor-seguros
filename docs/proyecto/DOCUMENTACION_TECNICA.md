...
- `app/ui/expiration_table_model.py`: adaptador y modelo Qt de solo lectura para presentar vencimientos calculados sin duplicar reglas de dominio.
- `app/ui/expiration_view.py`: pestaña de vencimientos con resumen agregado y tabla de consulta.
...
- `app/ui/edit_dialog.py`: ventana modal de edición controlada para aplicar cambios solo en memoria.
...
La fase `1.13.1` agrega una pestaña `Vencimientos` de solo lectura entre `Registros` y `Resumen`. Muestra total evaluadas, vigentes, próximas a vencer, vencidas, sin fecha válida y no aplica aviso. La tabla presenta póliza, asegurado, tipo, vigencia, fecha de vencimiento, estado, días restantes, prima, teléfono y correo. Usa `alert_days=30` fijo por ahora y conserva `D.M.` como `No aplica aviso`.

Estas fases no agregan avisos, exportaciones, documentos DOCX ni cambios en reglas de edición. Tampoco modifican archivos Excel reales ni generan archivos en carpetas locales de datos.
...
- resuelve encabezados mediante alias internos sin reemplazar encabezados originales;
...
- permite `Guardar` para escribir sobre el archivo cargado después de crear respaldo automático obligatorio;
...
- mantiene la tabla principal en modo solo lectura.
...
La versión `v1.12.0-beta` documenta el primer release beta funcional del MVP. Consolida el flujo actual de carga, consulta, edición en memoria, validaciones, bitácora de sesión, guardado controlado, ayuda integrada y preparación preliminar para PyInstaller, sin agregar ejecutable final ni instalador.

El documento de release vive en `docs/releases/v1.12.0-beta.md` e incluye alcance funcional, exclusiones vigentes, checklist de validación y texto listo para GitHub Release.
...
La versión `1.10.0` agrega edición controlada de registros solo en memoria.
...
La versión `1.10.4` centraliza normalización de columnas, alias, detección de coberturas, formato visual de `Emisión` y claves canónicas para validación.

No implementa:

- guardado en Excel;
- persistencia;
- cambios visibles grandes de operación;
- edición de coberturas;
- eliminación de columnas o datos;
- exportaciones;
- reportes persistentes;
...
La versión `1.11.1` agrega `Guardar` para modificar el Control Cartera cargado. Antes de escribir, la app crea un respaldo automático obligatorio en `data/backups/guardado_control_cartera/`.

El guardado directo:

- usa fila real e índice real de columna;
- no usa póliza, nombre, cédula, placa ni otros campos editables como identificadores;
- conserva coberturas ocultas, hojas existentes, filas no modificadas y columnas no modificadas;
- no elimina datos;
