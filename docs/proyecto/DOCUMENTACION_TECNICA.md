...
- `app/ui/expiration_dialog.py`: ventana modal de vencimientos con resumen agregado y tabla de consulta.
...
La fase `1.13.1` agrega una ventana `Vencimientos` de solo lectura. Muestra total evaluadas, vigentes, próximas a vencer, vencidas, sin fecha válida y no aplica aviso. La tabla presenta póliza, asegurado, tipo, vigencia, fecha de vencimiento, estado, días restantes, prima, teléfono y correo. Usa `alert_days=30` fijo por ahora y conserva `D.M.` como `No aplica aviso`.
