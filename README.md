...
No se implementa interfaz grafica funcional, base de datos operativa, importación real de Excel, generación real de documentos ni flujos operativos de vencimientos.
...
Estado de desarrollo posterior al release: la versión interna `1.13.1` conecta la lógica de vencimientos base con una pestaña inicial de solo lectura en la GUI. La vista no genera avisos, documentos, exportaciones ni modifica archivos Excel.
...
- `docs/proyecto/ESTANDARES_COLUMNAS_CONTROL_CARTERA.md`
...
Este chequeo técnico imprime nombre y versión, y sale de inmediato. No abre interfaz grafica, no resuelve rutas operativas, no lee ni escribe el workbook real, no crea persistencia y no ejecuta transformaciones de negocio.
...
La ventana muestra `Gestor de Seguros- Dagoberto Quirós Madriz`, deja lista la ruta predeterminada `data/input/CONTROLCARTERA_V2.xlsx`, permite cargarla con `Cargar predeterminado` y también permite seleccionar otro `.xlsx`. La pestaña `Registros` queda primero y muestra solo columnas originales visibles y filas útiles en una tabla de solo lectura. Las columnas de coberturas se conservan en memoria, pero se ocultan de tabla, detalle, edición y búsqueda. También permite buscar en todas las columnas visibles o en una columna específica, limpiar la búsqueda y ver el contador `Mostrando X de Y registros`. Al hacer doble clic sobre una fila, se abre `Detalle del registro` con los campos que contienen información. Desde ese detalle se puede abrir `Editar registro` para aplicar cambios en memoria; algunos campos usan controles específicos. Los errores bloqueantes impiden aplicar cambios y las advertencias suaves permiten revisar o aplicar de todos modos. La normalización de encabezados y alias se resuelve internamente sin reemplazar los encabezados originales. La pestaña `Vencimientos` muestra una evaluación inicial de vencimientos en modo solo lectura. La pestaña `Resumen` muestra conteos y estado de carga. La pestaña `Bitácora` muestra cambios registrados durante la sesión, también solo en memoria. `Guardar` modifica el archivo cargado con respaldo automático previo y `Guardar como` crea una copia `.xlsx`. `Ayuda` también puede abrirse con `F1`; `Ctrl+S` ejecuta `Guardar` y `Ctrl+Shift+S` ejecuta `Guardar como`. El botón compacto de tema permite alternar entre claro y oscuro, recordando la preferencia localmente. La app no elimina registros ni columnas.
...
- Gestión de clientes, pólizas, vencimientos y bitácoras persistentes o pistas de auditoría.
...
4. Definir el alcance de bitácoras persistentes y recuperación desde respaldos para una fase futura.
