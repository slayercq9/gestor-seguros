...
Desde `v1.11.2`, la GUI incluye accesos visibles a `Ayuda` y `Acerca de`. La ayuda resume carga del Control Cartera, búsqueda, detalle, edición, `Guardar`, `Guardar como`, respaldos y errores por archivo abierto en Excel. También se mantienen `Guardar` y `Guardar como`: `Guardar` escribe los cambios pendientes sobre el Control Cartera cargado después de crear un respaldo automático obligatorio en `data/backups/guardado_control_cartera/`, mientras `Guardar como` exporta una copia `.xlsx` sin sobrescribir el archivo cargado.
