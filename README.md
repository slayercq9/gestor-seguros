...
Desde `v1.11.1`, la GUI incluye `Guardar` y `Guardar como`. `Guardar` escribe los cambios pendientes sobre el Control Cartera cargado después de crear un respaldo automático obligatorio en `data/backups/guardado_control_cartera/`. `Guardar como` exporta una copia `.xlsx` sin sobrescribir el archivo cargado. Ambas rutas conservan coberturas ocultas, hojas existentes y datos no modificados; la bitácora sigue siendo solo en memoria.
