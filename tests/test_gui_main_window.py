...
def test_error_del_loader_no_expone_traza_tecnica(qapp):
    ...
    with workspace_tempdir() as temp_dir:
        ...
        assert "CONTROLCARTERA" not in window.warnings_text.toPlainText()
        assert "Traceback" not in window.warnings_text.toPlainText()
        assert "No se pudo cargar el Control Cartera" in window.statusBar().currentMessage()
        assert window.records_table.model().rowCount() == 0


def test_cancelar_selector_muestra_mensaje_amigable(qapp, monkeypatch):
    messages: list[str] = []
    monkeypatch.setattr(
        "app.ui.main_window.QFileDialog.getOpenFileName",
        lambda *args, **kwargs: ("", ""),
